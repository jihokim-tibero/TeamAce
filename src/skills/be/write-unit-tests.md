...
| 층위 | 책임 | 담당 |
|------|------|------|
| **단위 테스트** | service/domain/validation/error-branch | **BE** |
| 통합 테스트 | 리포지토리+DB, 서비스+외부 의존 | BE (선택) |
| E2E 테스트 | 브라우저·API 레벨 시나리오 | QA |

BE가 E2E를 작성하지 않는다. QA가 브라우저/HTTP 레벨에서 검증한다. BE는 "내부 로직이 명세대로 동작한다"만 증명한다.
...
### 4. 외부 의존 격리
- DB: 단위 테스트는 리포지토리 인터페이스를 fake/stub으로 치환. 실제 DB는 통합 테스트에서만.
...
- 단위 테스트 실행 시간: 전체 < 30초 (느려지면 통합 테스트로 이동 검토)
...
## 안티패턴
- 서비스 테스트에서 실제 DB 띄움 → 단위 테스트가 아닌 통합 테스트
- happy path만 쓰고 에러 분기 누락
- 랜덤/시간에 의존해 간헐적 실패 유발(flaky)
- E2E 성격의 테스트를 단위로 분류 (브라우저·전체 요청/응답 검증)
