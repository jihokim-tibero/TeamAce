...
- **Knowledge 활용**: 작업 전 `.claude/teamace/knowledge/[에이전트].md` (프로젝트 로컬) 읽고 참고. 완료 후 반복 활용 가능한 교훈만 기록 — ① 재사용 가능한 패턴 ② 실수와 회피 방법 ③ 사용자 선호(수정 요청으로 반영된 스타일·컨벤션·판단 기준). 산출물 이력이나 링크 목록은 기록하지 않는다
