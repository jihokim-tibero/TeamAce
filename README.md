...
### 4. CI 잡 자동 주입 (수동 트리거 전용)
GitLab은 `.gitlab-ci.yml`에, GitHub은 `.github/workflows/teamace-lint.yml`에 모듈별 lint 잡을 자동 주입합니다. **push/PR에서는 자동 실행되지 않고 web UI(GitLab "Run pipeline" / GitHub "Run workflow")에서 수동으로 trigger할 때만 실행**됩니다. **단일 진실 게이트는 로컬 pre-commit hook**.
...
lint/타입 검사는 **여러 시점에 layered**로 돌아 각 단계의 비용·신뢰도를 분담합니다. TeamAce는 **로컬에서 모두 잡는 정책**을 기본으로 하고, CI는 web UI 수동 trigger 전용입니다.

| 시점 | 목적 | 우회 가능성 |
|---|---|---|
| **에디터/IDE** | 실시간 피드백 | TeamAce 범위 밖 |
| **pre-commit hook** | **로컬에서 모든 lint/타입을 잡는 게이트** — 단일 진실 | `--no-verify` 우회 가능 (사용자 책임) |
| **CI lint 잡 (수동 trigger)** | 환경 재현 / 정밀 검증 시 사용자가 web UI에서 직접 실행 | push/PR 자동 실행 없음 |
...
#### CI lint 잡 (자동 주입 — 수동 트리거 전용)

push/PR 자동 실행을 끄고 **사용자가 web UI에서 수동 trigger했을 때만** 실행되도록 설정합니다. 로컬 pre-commit이 게이트이므로 CI는 환경 재현·정밀 검증 용도.
...
- **trigger rule**: `if: $CI_PIPELINE_SOURCE == "web"` + `when: manual` — web에서 "Run pipeline"으로 trigger한 경우에만 잡이 등장하고 그것도 manual 상태(한 번 더 click 필요). push/PR/schedule에서는 잡 자체 미생성

**GitHub Actions — `.github/workflows/teamace-lint.yml`**
- 파일이 없으면 새로 생성
- 있으면 skip (사용자가 직접 관리한다고 간주)
- 잡 이름: `teamace-lint-<modulepath>`
- **trigger**: `on: workflow_dispatch` 만 — Actions 탭의 "Run workflow"로 사용자 trigger. push/PR 자동 실행 없음
