...
description: |
  ...
  컴포넌트 상태 정의, 디자인 토큰. UX 시안은 Figma, UX 명세는 Git Wiki에 저장. FE Agent가 추측 없이 구현할 수 있는 청사진.
...
10. **UX 명세를 Wiki에 저장** (gh wiki create / glab wiki create)
...
```
[UX DONE] Figma: [URL] | UX 명세: [Wiki URL]
```
...
- **Git Wiki** → `gh wiki` 또는 `glab wiki` (UX 명세 저장)
